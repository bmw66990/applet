        ...
        public function login_action(Request $request) {
            ...
            if (Session::get('login_captcha') == $code) {
                    ...
                    switch ($request->get('type')) {
                        ...
                        case 4:
                            ...
                            if (Auth::guard($guard)->attempt([
                                ...
                                $rule = 1;
                                //return redirect()->intended('/admin');
                            } else if (Auth::guard($guard)->attempt([
                ...
                if ($rule == 1) {
                    ...
                    $is_get_today_experience    = object_array(DB::table('experiences')->where($login_condition)->whereBetween('created_at', [
                        $startTime,
                        $endTime
                    ])->count());
                    if ($is_get_today_experience == 0) {
                        ...
                        // 带有替换信息的上下文数组，键名为占位符名称，键值为替换值。
                        $experience_context = interpolate($experiencetpl, array(
